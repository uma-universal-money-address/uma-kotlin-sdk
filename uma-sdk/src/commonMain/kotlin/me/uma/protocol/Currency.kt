...
sealed interface Currency {
    ...
    /**
     * The number of digits after the decimal point for display on the sender side, and to add clarity around what the
     * "smallest unit" of the currency is. For example, in USD, by convention, there are 2 digits for cents - $5.95. In
     * this case, `decimals` would be 2. Note that the multiplier is still always in the smallest unit (cents). In
     * addition to display purposes, this field can be used to resolve ambiguity in what the multiplier means. For
     * example, if the currency is "BTC" and the multiplier is 1000, really we're exchanging in SATs, so `decimals`
     * would be 8.
     *
     * For details on edge cases and examples, see
 ...
 * @param minSendable Minimum amount that can be sent in this currency. This is in the smallest unit of the currency
 *   (eg. cents for USD).
 * @param maxSendable Maximum amount that can be sent in this currency. This is in the smallest unit of the currency
 *   (eg. cents for USD).
  ...
  senderUmaVersion: String = UMA_VERSION_STRING,
    ...
    return if (Version.parse(senderUmaVersion).major < 1) {
        ...
        CurrencyV1(
          ...
          symbol = symbol,
          millisatoshiPerUnit = millisatoshiPerUnit,
          convertible = CurrencyConvertible(min = minSendable, max = maxSendable),
          decimals = decimals,
        )
    }
}
...
internal data class CurrencyV1(
  ...
  @SerialName("multiplier") override val millisatoshiPerUnit: Double,
...
internal data class CurrencyV0(
  ...
  @SerialName("multiplier") override val millisatoshiPerUnit: Double,
  /**
   * Minimum amount that can be sent in this currency. This is in the smallest unit of the currency (eg. cents for USD).
   */
  val minSendable: Long,
  /**
   * Maximum amount that can be sent in this currency. This is in the smallest unit of the currency (eg. cents for USD).
   */
