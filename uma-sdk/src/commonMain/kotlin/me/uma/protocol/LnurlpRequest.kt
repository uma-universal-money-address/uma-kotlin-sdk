 ...
 * @param isSubjectToTravelRule Indicates whether the VASP1 is a financial institution that requires travel rule
 *   information.
 * @param vaspDomain The domain of the VASP that is sending the payment. It will be used by VASP2 to fetch the public
 *   keys of VASP1.
...
data class LnurlpRequest(
  ...
  val backingSignatures: List<BackingSignature>? = null,
              ...
              parameters =
                Parameters.build {
                    ...
                    backingSignatures?.let { signatures ->
                        append("backingSignatures", signatures.joinToString(",") { "${it.domain}:${it.signature}" })
                    }
                },
            )
            .build()
    ...
    fun asUmaRequest(): UmaLnurlpRequest? {
        return if (
          nonce != null && signature != null && vaspDomain != null && timestamp != null && umaVersion != null
        ) {
            UmaLnurlpRequest(
              receiverAddress,
              nonce,
    ...
    companion object {
        ...
        fun decodeFromUrl(url: String): LnurlpRequest {
            ...
            if (
              urlBuilder.pathSegments.size != 4 ||
            ...
            val backingSignatures =
              urlBuilder.parameters["backingSignatures"]?.let { serialized ->
