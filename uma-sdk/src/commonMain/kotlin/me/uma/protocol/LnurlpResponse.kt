 ...
 *   request.
 * @property nostrPubkey An optional nostr pubkey used for nostr zaps (NIP-57). If set, it should be a valid BIP-340
 *   public key in hex format.
 * @property allowsNostr Should be set to true if the receiving VASP allows nostr zaps (NIP-57).
 * @property backingSignatures A list of backing signatures from VASPs that can attest to the authenticity of the
 *   message.
 */
...
data class LnurlpResponse(
  ...
  @SerialName("payerData") val requiredPayerData: CounterPartyDataOptions?,
  val compliance: LnurlComplianceResponse?,
  val umaVersion: String?,
  @SerialName("commentAllowed") val commentCharsAllowed: Int? = null,
  val nostrPubkey: String? = null,
  val allowsNostr: Boolean? = null,
  @EncodeDefault val tag: String = "payRequest",
  val backingSignatures: List<BackingSignature>? = null,
) {
    fun asUmaResponse(): UmaLnurlpResponse? =
      if (currencies != null && requiredPayerData != null && compliance != null && umaVersion != null) {
          UmaLnurlpResponse(
            callback,
            minSendable,
            maxSendable,
