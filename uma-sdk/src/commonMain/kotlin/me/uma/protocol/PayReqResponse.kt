...
sealed interface PayReqResponse {
    ...
    /**
     * Information about the payment that the receiver will receive. Includes final currency-related information for the
     * payment. Required for UMA.
     */
    val paymentInfo: PayReqResponsePaymentInfo?

    /** Usually just an empty list from legacy LNURL, which was replaced by route hints in the BOLT11 invoice. */
...
internal data class PayReqResponseV1(
  @SerialName("pr") override val encodedInvoice: String,
  @SerialName("converted") override val paymentInfo: V1PayReqResponsePaymentInfo?,
  /** The data about the receiver that the sending VASP requested in the payreq request. Required for UMA. */
  val payeeData: PayeeData?,
  @EncodeDefault override val routes: List<Route> = emptyList(),
  /**
   * This field may be used by a WALLET to decide whether the initial LNURL link will be stored locally for later reuse
   * or erased. If disposable is null, it should be interpreted as true, so if SERVICE intends its LNURL links to be
   * stored it must return `disposable: false`. UMA should always return `disposable: false`. See LUD-11.
   */
  ...
  val successAction: Map<String, String>? = null,
    ...
    override fun isUmaResponse() =
      payeeData != null && payeeData.payeeCompliance() != null && payeeData.identifier() != null && paymentInfo != null

    @Throws(UmaException::class)
    fun signablePayload(payerIdentifier: String): ByteArray {
        if (payeeData == null) {
            throw UmaException("Payee data is required for UMA", ErrorCode.MISSING_REQUIRED_UMA_PARAMETERS)
        }
        if (payeeData.identifier() == null) {
            throw UmaException("Payee identifier is required for UMA", ErrorCode.MISSING_REQUIRED_UMA_PARAMETERS)
        }
        val complianceData =
...
@Serializable data class Route(val pubkey: String, val path: List<RouteHop>)

@Serializable data class RouteHop(val pubkey: String, val channel: String, val fee: Long, val msatoshi: Long)
...
sealed interface PayReqResponsePaymentInfo {
    ...
    /**
     * Number of digits after the decimal point for the receiving currency. For example, in USD, by convention, there
     * are 2 digits for cents - $5.95. In this case, `decimals` would be 2. This should align with the currency's
     * `decimals` field in the LNURLP response. It is included here for convenience. See
    ...
    /**
     * The conversion rate. It is the number of millisatoshis that the receiver will receive for 1 unit of the specified
     * currency (eg: cents in USD). In this context, this is just for convenience. The conversion rate is also baked
     * into the invoice amount itself. Specifically: `invoiceAmount = amount * multiplier + exchangeFeesMillisatoshi`
     */
    val multiplier: Double

    /**
     * The fees charged (in millisats) by the receiving VASP for this transaction. This is separate from the
     * [multiplier].
     */
...
internal data class V1PayReqResponsePaymentInfo(
  ...
  @SerialName("fee") override val exchangeFeesMillisatoshi: Long,
 ...
 */
@Serializable
data class PayReqResponseCompliance(val utxos: List<String>, val nodePubKey: String?, val utxoCallback: String)
