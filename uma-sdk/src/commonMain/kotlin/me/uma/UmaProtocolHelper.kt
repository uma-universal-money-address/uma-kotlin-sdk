...
/**
 * A helper class for interacting with the UMA protocol. It provides methods for creating and verifying UMA requests and
 * responses.
 */
...
constructor(
  ...
  private val umaRequester: UmaRequester = KtorUmaRequester(),
    ...
    fun fetchPublicKeysForVaspFuture(vaspDomain: String): Future<PubKeyResponse> =
      coroutineScope.future { fetchPublicKeysForVasp(vaspDomain) }
     ...
     *   sign the request.
     * @param receiverAddress The address of the user at the receiving VASP that is receiving the payment (i.e.
     *   $bob@vasp2.com).
     ...
     * @param isSubjectToTravelRule Indicates whether the sending VASP is a financial institution that requires travel
     *   rule information.
     * @param umaVersion The version of the UMA protocol that the sending VASP prefers to use for this transaction. This
     *   parameter should only be overridden in cases where the receiving VASP does not support the default version for
     *   this SDK. For the version negotiation flow, see
    ...
    fun verifyUmaLnurlpQuerySignature(
      ...
      nonceCache: NonceCache,
        ...
        return verifySignature(query.signablePayload(), query.signature, pubKeyResponse.getSigningPublicKey())
    }
     ...
     *   request.
     * @param nostrPubkey An optional nostr pubkey used for nostr zaps (NIP-57). If set, it should be a valid BIP-340
    ...
    fun getLnurlpResponse(
      ...
      nostrPubkey: String? = null,
        ...
        val currencies =
          if (Version.parse(umaVersion).major < 1) {
              currencyOptions.map {
                  if (it is CurrencyV1) {
                      CurrencyV0(
                        code = it.code,
     ...
     * @param amount The amount that the receiver will receive in either the smallest unit of the receiving currency (if
     *   `isAmountInReceivingCurrency` is True), or in msats (if false).
     ...
     * @param comment A comment that the sender would like to include with the payment. This can only be included if the
     *   receiver included the `commentAllowed` field in the lnurlp response. The length of the comment must be less
     *   than or equal to the value of `commentAllowed`.
     * @param receiverUmaVersion The UMA version of the receiver VASP. This information can be obtained from the
     *   [LnurlpResponse]
    ...
    fun getPayRequest(
      ...
      receiverUmaVersion: String = UMA_VERSION_STRING,
        ...
        val compliancePayerData =
          getSignedCompliancePayerData(
        ...
        if (Version.parse(receiverUmaVersion).major < 1) {
            return PayRequestV0(currencyCode = receivingCurrencyCode, amount = amount, payerData = payerData)
        } else {
            return PayRequestV1(
    ...
    private fun getSignedCompliancePayerData(
      ...
      travelRuleFormat: TravelRuleFormat?,
        ...
        val nonce = generateNonce()
        val timestamp = System.currentTimeMillis() / 1000
        val unsignedCompliancePayerData =
          CompliancePayerData(
    ...
    fun verifyPayReqSignature(payReq: PayRequest, pubKeyResponse: PubKeyResponse, nonceCache: NonceCache): Boolean {
        ...
        return verifySignature(payReq.signablePayload(), compliance.signature, pubKeyResponse.getSigningPublicKey())
    }
     ...
     * @param receivingCurrencyCode The code of the currency that the receiver will receive for this payment.
     * @param receivingCurrencyDecimals The number of digits after the decimal point for the receiving currency. For
     *   example, USD has 2 decimal places. This should align with the `decimals` field returned for the chosen currency
     *   in the LNURLP response.
     ...
     *   invoice expires.
     * @param receiverFeesMillisats The fees charged (in millisats) by the receiving VASP for this transaction. This is
     *   separate from the [conversionRate].
     ...
     * @param utxoCallback The URL that the receiving VASP will call to send UTXOs of the channel that the receiver used
     *   to receive the payment once it completes.
     ...
     *   intends its LNURL links to be stored it must return `disposable: false`. UMA should always return `disposable:
     *   false`. See LUD-11.
    ...
    ): CompletableFuture<PayReqResponse> =
    ...
    fun getPayReqResponseSync(
      ...
      senderUmaVersion: String = UMA_VERSION_STRING,
        ...
        val futureInvoiceCreator =
          object : UmaInvoiceCreator {
    ...
    suspend fun getPayReqResponse(
      ...
      senderUmaVersion: String = UMA_VERSION_STRING,
        ...
        val metadataWithInvoiceUUID = query.invoiceUUID()?.let { addInvoiceUUIDtoMetadata(metadata, it) } ?: metadata
        ...
        val requiredUmaFields: Map<String, Any?> =
          mapOf(
        ...
        if (query.isUmaRequest()) {
            mutablePayeeData["compliance"] =
              serialFormat.encodeToJsonElement(
    ...
    private fun getSignedCompliancePayeeData(
      ...
      utxoCallback: String,
        ...
        val nonce = generateNonce()
        val timestamp = System.currentTimeMillis() / 1000
        val unsignedCompliancePayeeData =
          CompliancePayeeData(
    ...
    fun verifyUmaInvoice(invoice: Invoice, pubKeyResponse: PubKeyResponse): Boolean {
        return invoice.signature?.let { signature ->
            verifySignature(invoice.toSignablePayload(), signature, pubKeyResponse.getSigningPublicKey())
        } ?: false
    }
    ...
    fun getInvoice(
      ...
      requiredPayerData: CounterPartyDataOptions? = null,
        ...
        return Invoice(
          ...
          .apply { signature = Secp256k1.signEcdsa(toSignablePayload(), privateSigningKey) }
    }
}
