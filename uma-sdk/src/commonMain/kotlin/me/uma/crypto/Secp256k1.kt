...
object Secp256k1 {
    fun signEcdsa(message: ByteArray, privateKey: ByteArray): ByteArray {
        return me.uma.crypto.internal.signEcdsa(
            message.toUByteArray().toList(),
            privateKey.toUByteArray().toList(),
        ).toByteArray()
    }
    ...
    fun encryptEcies(message: ByteArray, publicKey: ByteArray): ByteArray {
        return me.uma.crypto.internal.encryptEcies(
            message.toUByteArray().toList(),
            publicKey.toUByteArray().toList(),
        ).toByteArray()
    }
    ...
    fun decryptEcies(message: ByteArray, privateKey: ByteArray): ByteArray {
        return me.uma.crypto.internal.decryptEcies(
            message.toUByteArray().toList(),
            privateKey.toUByteArray().toList(),
        ).toByteArray()
    }
    ...
    fun generateKeyPair(): KeyPair {
        ...
        return KeyPair(
            keyPair.getPublicKey().toByteArray(),
            keyPair.getPrivateKey().toByteArray(),
        )
    }
}
