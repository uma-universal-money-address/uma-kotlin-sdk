...
public class UmaTest {
  ...
  UmaProtocolHelper umaProtocolHelper =
      new UmaProtocolHelper(publicKeyCache, new TestUmaRequester());
  ...
  private static final String PRIVKEY_HEX =
      "77e891f0ecd265a3cda435eaa73792233ebd413aeb0dbb66f2940babfc9a2667";
  private static final String encodedPayReqMetadata =
      "[[\"text/uma-invoice\",\"invoiceUUID\"],[\"text/plain\",\"otherInformations\"]]";
  ...
  public void testFetchPublicKeyFuture() throws Exception {
    PubKeyResponse pubKeys =
        umaProtocolHelper.fetchPublicKeysForVaspFuture("https://vasp.com").get();
    assertNotNull(pubKeys);
    System.out.println(pubKeys);
  }
  ...
  public void testGetLnurlpRequest() throws Exception {
    String lnurlpUrl =
        umaProtocolHelper.getSignedLnurlpRequestUrl(
            privateKeyBytes(),
    ...
    assertTrue(
        umaProtocolHelper.verifyUmaLnurlpQuerySignature(
            requireNonNull(request.asUmaRequest()),
            new PubKeyResponse(publicKeyBytes(), publicKeyBytes()),
  ...
  public void testGetLnurlpResponse_umaV1() throws Exception {
    String lnurlpUrl =
        umaProtocolHelper.getSignedLnurlpRequestUrl(
            privateKeyBytes(), "$bob@vasp2.com", "https://vasp.com", true);
    LnurlpRequest request = umaProtocolHelper.parseLnurlpRequest(lnurlpUrl);
    assertNotNull(request);

    LnurlpResponse lnurlpResponse =
        umaProtocolHelper.getLnurlpResponse(
            request,
            privateKeyBytes(),
            true,
            ...
            CounterPartyData.createCounterPartyDataOptions(
                Map.of(
                    "name", false,
                    "email", false)),
            List.of(createCurrency("USD", "US Dollar", "$", 34_150, 2, 1, 10_000_000, "1.0")),
            KycStatus.VERIFIED);
    ...
    CounterPartyDataOption complianceOption =
        parsedResponse.asUmaResponse().getRequiredPayerData().get("compliance");
    assertNotNull(complianceOption);
    assertTrue(complianceOption.getMandatory());
    CounterPartyDataOption identifierOption =
        parsedResponse.asUmaResponse().getRequiredPayerData().get("identifier");
    assertNotNull(identifierOption);
    assertTrue(identifierOption.getMandatory());
    assertTrue(
        umaProtocolHelper.verifyLnurlpResponseSignature(
            requireNonNull(parsedResponse.asUmaResponse()),
            new PubKeyResponse(publicKeyBytes(), publicKeyBytes()),
            new InMemoryNonceCache(1L)));
  }
        ...
        umaProtocolHelper.getLnurlpResponse(
            ...
            CounterPartyData.createCounterPartyDataOptions(
                Map.of(
                    ...
                    "identity", true,
                    "compliance", true)),
            List.of(createCurrency("USD", "US Dollar", "$", 34_150, 2, 1, 10_000_000, "0.3")),
            KycStatus.VERIFIED);
    assertNotNull(lnurlpResponse);
    String responseJson = lnurlpResponse.toJson();
    System.out.println(responseJson);
  ...
  public void testSignAndVerifyLnurlpResponseWithBackingSignature() throws Exception {
    ...
    UmaLnurlpResponse responseWithBackingSignature =
        umaResponse.appendBackingSignature(privateKeyBytes(), backingDomain);
    ...
    PubKeyResponse backingVaspPubKeyResponse =
        new PubKeyResponse(publicKeyBytes(), publicKeyBytes(), publicKeyCacheExpiry);
  ...
  public void testGetPayRequest_umaV1() throws Exception {
    ...
    System.out.println(request);
    assertTrue(
        umaProtocolHelper.verifyPayReqSignature(
            request,
            new PubKeyResponse(publicKeyBytes(), publicKeyBytes()),
            new InMemoryNonceCache(1L)));
  ...
  public void testSignAndVerifyPayReqBackingSignatures() throws Exception {
    ...
    String backingDomain = "backingvasp.com";
    PayRequest requestWithBackingSignatures =
        request.appendBackingSignature(privateKeyBytes(), backingDomain);
    ...
    CompliancePayerData complianceData =
        getSerialFormat()
            .decodeFromJsonElement(
                CompliancePayerData.Companion.serializer(), payerData.get("compliance"));
  ...
  public void testGetPayReqResponseSync_umaV1() throws Exception {
    PayRequest request =
        umaProtocolHelper.getPayRequest(
            ...
            KycStatus.VERIFIED,
            "");
    PayReqResponse response =
        umaProtocolHelper.getPayReqResponseSync(
            request,
    ...
    assertTrue(
        umaProtocolHelper.verifyPayReqResponseSignature(
            response,
            new PubKeyResponse(publicKeyBytes(), publicKeyBytes()),
            "$alice@vasp1.com",
            new InMemoryNonceCache(1L)));
    ...
    JsonObject json =
        Json.Default.decodeFromString(JsonObject.Companion.serializer(), responseJson);
  ...
  public void testSignAndVerifyPayReqResponseBackingSignatures() throws Exception {
    ...
    PayReqResponse response =
        umaProtocolHelper.getPayReqResponseSync(
            ...
            "1.0");
    String backingDomain = "backingvasp.com";
    PayReqResponse signedResponse =
        response.appendBackingSignature(
            privateKeyBytes(), backingDomain, "$alice@vasp1.com", "$bob@vasp2.com");
    String responseJson = signedResponse.toJson();
    PayReqResponse parsedResponse = umaProtocolHelper.parseAsPayReqResponse(responseJson);
    assertNotNull(parsedResponse);
    JsonObject payeeData = parsedResponse.payeeData();
    assertNotNull(payeeData);
    CompliancePayeeData complianceData =
        getSerialFormat()
            .decodeFromJsonElement(
                CompliancePayeeData.Companion.serializer(), payeeData.get("compliance"));
    assertNotNull(complianceData);
    assertNotNull(complianceData.getBackingSignatures());
    assertEquals(1, complianceData.getBackingSignatures().size());
    Long publicKeyCacheExpiry = System.currentTimeMillis() / 1000 + 10000;
    ...
    publicKeyCache.addPublicKeysForVasp(backingDomain, backingVaspPubKeyResponse);
    assertTrue(
        umaProtocolHelper.verifyPayReqResponseBackingSignaturesSync(
            parsedResponse, "$alice@vasp1.com"));
  }
  ...
  public void testVerifyUmaLnurlpQuerySignature_oldSignature() throws Exception {
    ...
    InMemoryNonceCache nonceCache =
        new InMemoryNonceCache(System.currentTimeMillis() / 1000 + 1000);
  ...
  public void testSignAndVerifyLnurlpRequestWithBackingSignature() throws Exception {
    ...
    String backingDomain = "backingvasp.com";
    UmaLnurlpRequest requestWithBackingSignature =
        request.asUmaRequest().appendBackingSignature(backingVaspPrivateKey, backingDomain);
    ...
    assertTrue(
        umaProtocolHelper.verifyUmaLnurlpQuerySignature(
            ...
            new InMemoryNonceCache(1L)));
    assertTrue(
        umaProtocolHelper.verifyUmaLnurlpQueryBackingSignaturesSync(
  ...
  public void testGetAndVerifyPostTransactionCallback() throws Exception {
    PostTransactionCallback callback =
        umaProtocolHelper.getPostTransactionCallback(
            Arrays.asList(new UtxoWithAmount("utxo1", 1000), new UtxoWithAmount("utxo2", 2000)),
    ...
    assertTrue(
        umaProtocolHelper.verifyPostTransactionCallbackSignature(
            parsedCallback,
            new PubKeyResponse(publicKeyBytes(), publicKeyBytes()),
            new InMemoryNonceCache(1L)));
  }
...
class TestUmaRequester implements UmaRequester {
  @Nullable
  @Override
  public Object makeGetRequest(
      @NotNull String url, @NotNull Continuation<? super String> $completion) {
...
class TestUmaInvoiceCreator implements UmaInvoiceCreator {
  ...
  public CompletableFuture<String> createUmaInvoice(
      long amountMsats, @NotNull String metadata, @Nullable String receiverIdentifier) {
